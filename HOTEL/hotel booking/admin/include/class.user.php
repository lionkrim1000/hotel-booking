    ...
    include "db_config.php";
        class user
        {
            ...
            public function add_room($roomname, $room_qnty, $no_bed, $bedtype,$facility,$price)
            {
                    ...
                    {
                        $sql2="INSERT INTO bookings SET room_cat='$roomname', book='false'";
            ...
            public function check_available($checkin, $checkout)
            {
                   ...
                   $sql="SELECT DISTINCT room_cat FROM bookings WHERE room_id NOT IN (SELECT DISTINCT room_id
   FROM bookings WHERE (checkin <= '$checkin' AND checkout >='$checkout') OR (checkin >= '$checkin' AND checkin <='$checkout') OR (checkin <= '$checkin' AND checkout >='$checkin') )";
            ...
            public function booknow($checkin, $checkout, $name, $phone,$roomname)
            {
                    ...
                    $sql="SELECT * FROM bookings WHERE room_cat='$roomname' AND (room_id NOT IN (SELECT DISTINCT room_id
   FROM bookings WHERE checkin <= '$checkin' AND checkout >='$checkout'))";
                    ...
                    if(mysqli_num_rows($check) > 0)
                    {
                        ...
                        $sql2="UPDATE bookings  SET checkin='$checkin', checkout='$checkout', name='$name', phone='$phone', book='true' WHERE room_id=$id";
             ...
             public function edit_room_cat($roomname, $room_qnty, $no_bed, $bedtype,$facility,$price,$room_cat)
            {
                        ...
                        $sql2="DELETE FROM bookings WHERE room_cat='$room_cat'";
                        mysqli_query($this->db,$sql2);
                 
                 
                        for($i=0;$i<$room_qnty;$i++)
                        {
                            $sql3="INSERT INTO bookings SET room_cat='$roomname', book='false'";
